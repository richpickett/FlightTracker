...
exports.handler = async (event) => {
  ...
  for (const el of (data.elements || [])) {
    if (el.tags && el.tags.aeroway === "aerodrome" && el.geometry && el.geometry.length) {
      ads.push({ icao: (el.tags.icao || el.tags.faa || el.tags.iata || "").toUpperCase(), c: centroid(el.geometry) });
    }
  }
  ...
  // Keep a feature only if its midpoint's nearest aerodrome is the target. No target/aerodromes -> keep all (old behaviour).
  const keep = geom => { if (!target || !ads.length) return true; const m = geom[Math.floor(geom.length / 2)]; return nearestAd([m.lat, m.lon]) === target; };
